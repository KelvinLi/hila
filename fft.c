...
static void radf2(int ido, int l1, const double cc[], double ch[], const double wa1[])
  {
    int i, k, ic;
    double ti2, tr2;
    for (k=0; k<l1; k++) {
 ...
 */
static void
factorize (int n, int ifac[MAXFAC + 2], const int ntryh[NSPECIAL])
{
...
static void
rffti1 (int n, double *wa, int ifac[MAXFAC+2])
{
    static const double twopi = 6.28318530717959;
    ...
    static const int ntryh[NSPECIAL] = {4, 2, 3, 5};
...
/* Returns a newly allocated array for DFT input size of `n'. */
double *
fft_initialize (int n)
{
    int dim;
    double * wsave;
    ...
    dim = 2 * n + 15;
    wsave = malloc (sizeof (*wsave) * (unsigned int) dim);
    if (!wsave)
        return NULL;
    rffti (n, wsave);
    return wsave;
...
/* `n' should be length of data array, which should be same as the `n'
 * given to `fft_initialize' to create `wsave'.
 */
double *
fft_compute (int n, double *data, double *wsave)
{
    /* interleaved (real, imag) complex values */
    double *ret;

    ret = malloc (2 * sizeof (double) * (unsigned int) (n / 2 + 1));
    if (!ret)
        return NULL;
    memcpy ((char *) (ret + 1), data, sizeof (double) * (unsigned int) n);
    rfftf (n, ret + 1, wsave);

    /* twiddling real and imag components of first value */
    ret[0] = ret[1];
    ret[1] = 0.0;
    return ret;
}
